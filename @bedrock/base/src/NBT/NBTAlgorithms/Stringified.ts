...
class StringifiedNBTDefinitionWriter extends DefinitionWriter {
	...
	public [NBTTag.Compoud](value: { [k: string]: NBTValue }, spacing?: string, depth: number = 0): void {
		this.text += "{";
		let hasBefore = false;
		const baseSpace = spacing ? spacing.repeat((depth ?? 0) + 1) : "";
		for (const [key, v] of Object.entries(value)) {
			if (!v[Symbol.NBT_TYPE]) continue;
			if (hasBefore) this.text += ",";
			if (spacing) this.text += "\n" + baseSpace;
			this.text += key + ":" + (spacing ? " " : "");
			v[Symbol.NBT_WRITE](this, spacing, (depth ?? 0) + 1);
			hasBefore = true;
		}
	...
	public [NBTTag.List](value: NBTValue[], spacing?: string, depth: number = 0): void {
		this.text += "[";
		let hasBefore = false;
		const baseSpace = spacing ? spacing.repeat((depth ?? 0) + 1) : "";
	...
	public Stringify(tag: NBTValue, spacing?: string) {
		const type = tag[Symbol.NBT_TYPE];
		this.text = "";
...
class StringifiedNBTDefinitionReader extends DefinitionReader {
	...
	public [NBTTag.Compoud](): { [k: string]: NBTValue } {
		throw new Error("Method not implemented.");
	}
...
namespace SNBT {
	...
	function readArray(source: Source) {
		...
		for (let char of source) {
			...
			if (!(kind in kinds)) throw new TypeError("Unexpected kind: " + kind);
			const value = kinds[kind as "string"](source);
			if (!initialKind) initialKind = value[Symbol.NBT_TYPE];
			else if (initialKind !== value[Symbol.NBT_TYPE])
				throw new TypeError(
					"Array could have just one kind of elements, but multiple, expected: " +
						NBTTag[initialKind] +
						" but got: " +
						NBTTag[value[Symbol.NBT_TYPE]],
				);
			obj.push(value);
			readWhiteSpace(source);
			firsObj = false;
...
class StringifiedNBT extends NBT {
	public static Stringify(tag: NBTValue, spacing?: string) {
