...
import { AttributeComponent, EntityComponent, EntityComponentUpdateEventData, FloatEntityComponent, FloatEntityComponentValueChangeEventData } from "./BaseComponents.js";
...
export class ScaleComponent extends FloatEntityComponent<EntityComponentId.Scale> implements MetadataComponent{
	public constructor(entity: Entity){ super(entity, EntityComponentId.Scale); }	
	public *getMetadata() {
		...
		yield {
			...
			get value(){return data[componentId + ".value"]??1;}
		};
	}
}

export class AbsorptionComponent extends AttributeComponent<EntityComponentId.Absorption> {
	public constructor(entity: Entity){ super(entity, EntityComponentId.Absorption); }
}
export class AttackDamageComponent extends AttributeComponent<EntityComponentId.AttackDamage> implements MetadataComponent{
	public constructor(entity: Entity){ super(entity,EntityComponentId.AttackDamage); }
	public *getMetadata() {
		const data = this.entity.entityData;
		const componentId = this.componentId;
		yield {
		...
		yield {
			...
			type: MetadataType.Float,
			get value(){return data[componentId + ".max"]??FL32_MAX;}
		};
	}
}

export class FallDamageComponent extends AttributeComponent<EntityComponentId.FallDamage> implements MetadataComponent{
	public get multipliter(): number{return (this.entity).entityData[EntityDataKeys.FallDamageMultiplier]??1;}
	public set multipliter(v){ 
		const data = new FloatEntityComponentValueChangeEventData(this, this.entity.entityData[EntityDataKeys.FallDamageMultiplier], v);
	...
	public constructor(entity: Entity){ super(entity, EntityComponentId.FallDamage); }
	public *getMetadata() {
		const data = this.entity.entityData;
		yield {
			key: MetadataKey.FallDamageMultiplier,
			type: MetadataType.Float,
			get value(){return data[EntityDataKeys.FallDamageMultiplier]??1;}
		};
	}
}
export class FollowRangeComponent extends AttributeComponent<EntityComponentId.FollowRange> {
	public constructor(entity: Entity){ super(entity, EntityComponentId.FollowRange); }
}

export class HorseJumpStrengthComponent extends AttributeComponent<EntityComponentId.HorseJumpStrength> {
	public constructor(entity: Entity){ super(entity, EntityComponentId.HorseJumpStrength); }
}
export class HealthComponent extends AttributeComponent<EntityComponentId.Health> implements MetadataComponent{
	public constructor(entity: Entity){ super(entity, EntityComponentId.Health); }
	public *getMetadata() {
		const data = this.entity.entityData;
		const componentId = this.componentId;
		yield {
...
class StatusPropertiesComponentPrivate extends EntityComponent<"minecraft:entity_status">{
	...
	public constructor(entity: Entity){
		...
		return new Proxy(this, {
			has(target, p) { 
				return Object.hasOwn(MetadataFlags, typeof p === "string" && p.startsWith("is")?p.slice(2):p) || Reflect.has(target, p); 
			},
			ownKeys(target) {
			...
			set(t, p, v){
				...
				if(Object.hasOwn(MetadataFlags, pp)) {
					t.currentValue = v?t.currentValue | MetadataFlags[pp as "AffectedByGravity"]:t.currentValue & ~MetadataFlags[pp as "AffectedByGravity"];
					return true;
				}
	...
	public *getMetadata() {
		...
		yield {
			...
			get value(){return data[componentId + ".value"]??0;}
		};
	}
}
...
export const StatusPropertiesComponent: new ()=>(StatusPropertiesComponent & StatusPropertiesComponentPrivate) = StatusPropertiesComponentPrivate as unknown as any;
...
export type ComponentMapInstances = {[k in keyof typeof ComponentMapConstructors]: InstanceType<typeof ComponentMapConstructors[k]> };
